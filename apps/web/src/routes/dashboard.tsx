...
export function Dashboard() {
  ...
  const heroStats = [
    { label: "Agentes", value: stats?.activeAgents ?? 0, icon: Users, color: "text-primary", bg: "bg-primary-light" },
    { label: "Em Progresso", value: stats?.runningTasks ?? 0, icon: Clock, color: "text-yellow", bg: "bg-yellow-light" },
    { label: "Em Review", value: stats?.reviewTasks ?? 0, icon: AlertCircle, color: "text-purple", bg: "bg-purple-light" },
    { label: "Concluídas", value: stats?.doneTasks ?? 0, icon: CheckCircle2, color: "text-green", bg: "bg-green-light" },
  ];
    ...
    <div className="p-8">
      <div className="stagger flex flex-col gap-8">
          ...
          <div className="rounded-3xl bg-gradient-to-r from-hero-from to-hero-to p-8 shadow-lg">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="mt-2 text-[14px] text-white/50 font-mono">{activeProject.path}</p>
              ...
              <div className="hidden lg:flex gap-6">
                <div className="text-center">
                  <p className="text-[28px] font-bold text-white">{stats?.activeAgents ?? 0}</p>
                  <p className="text-[12px] text-white/50">Agentes</p>
                </div>
                <div className="text-center">
                  <p className="text-[28px] font-bold text-white">{stats?.totalTasks ?? 0}</p>
                  <p className="text-[12px] text-white/50">Tasks</p>
                </div>
                <div className="text-center">
                  <p className="text-[28px] font-bold text-primary">AI</p>
                  <p className="text-[12px] text-white/50">Powered</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-3xl bg-white p-8 shadow-card">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 rounded-full bg-primary px-6 py-3 text-[14px] font-medium text-white shadow-sm transition-all duration-200 hover:bg-primary-hover hover:shadow-md active:scale-[0.98] disabled:opacity-40"
              >
        ...
        {projects.length > 0 && (
          <div className="rounded-2xl bg-white p-5 shadow-card">
              ...
              <div className="flex-1">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 rounded-xl border border-edge bg-page px-4 py-2.5 text-[13px] text-text-primary placeholder-text-placeholder outline-none transition-all focus:border-primary focus:ring-2 focus:ring-primary-muted"
                    onKeyDown={(e) => e.key === "Enter" && handleScan()}
                  />
                  <button
                    onClick={handleScan}
                    disabled={scanning || !workspacePath.trim()}
                    className="flex items-center gap-2 rounded-xl bg-primary px-5 py-2.5 text-[13px] font-medium text-white transition-all duration-200 hover:bg-primary-hover active:scale-[0.98] disabled:opacity-40"
                  >
                    {scanning ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}
                    Scan
                  </button>
            ...
            {scannedProjects.length > 0 && (
              <div className="mt-4 border-t border-edge-light pt-4">
                ...
                <div className="flex flex-col gap-2">
                  {scannedProjects.map((scanned) => {
                    ...
                    return (
                      <div
                        key={scanned.path}
                        className="flex items-center justify-between rounded-xl bg-page px-4 py-3 transition-colors hover:bg-surface-hover"
                      >
                        <div className="flex items-center gap-3 min-w-0">
                          <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary-light text-[11px] font-bold text-primary">
                            {scanned.icon}
                          </span>
                        ...
                        <button
                          ...
                          className={cn(
                            "ml-3 flex shrink-0 items-center gap-1.5 rounded-full px-4 py-1.5 text-[12px] font-medium transition-all",
                            alreadyAdded
                              ? "text-green"
        ...
        {stats && stats.recentActivities.length > 0 && (
          <section>
            <h3 className="mb-4 text-[18px] font-semibold text-text-primary">Atividades Recentes</h3>
            <div className="grid grid-cols-4 gap-4">
              {stats.recentActivities.slice(0, 8).map((activity) => (
                <div
                  key={activity.id}
                  className="rounded-[20px] bg-white p-5 shadow-card card-hover"
                >
                  <div className="flex items-center gap-3 mb-3">
                    <div
                    ...
                    <div className="min-w-0">
                      <p className="truncate text-[13px] font-semibold text-text-primary">{activity.agentName}</p>
                      <p className="text-[11px] text-text-tertiary">{formatRelativeTime(activity.createdAt)}</p>
                    </div>
                  </div>
                  ...
                  {activity.detail && (
                    <p className="mt-1 text-[11px] text-text-tertiary line-clamp-1">{activity.detail}</p>
                  )}
                </div>
              ))}
            </div>
        ...
        {projects.length > 0 && (
          <section>
            <h3 className="mb-4 text-[18px] font-semibold text-text-primary">Meus Projetos</h3>
              ...
              {projects.map((project) => {
                ...
                return (
                  <button
                    ...
                    className="group flex items-center gap-4 rounded-2xl bg-white p-5 text-left shadow-card card-hover"
                  >
                    <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-primary-light text-[14px] font-bold text-primary transition-all duration-200 group-hover:bg-primary group-hover:text-white group-hover:shadow-md">
                      {icon}
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[14px] font-semibold text-text-primary group-hover:text-primary transition-colors">{project.name}</p>
                      <div className="mt-1 flex flex-wrap gap-1.5">
                        {stack.slice(0, 3).map((tech) => (
                          <span key={tech} className="rounded-md bg-page px-2 py-0.5 text-[10px] font-medium text-text-tertiary">
                            {tech}
                          </span>
                        ))}
                      </div>
                    </div>
                    <ArrowUpRight className="h-4 w-4 shrink-0 text-text-placeholder transition-all duration-200 group-hover:text-primary group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
                  </button>
                );
              })}
            </div>
