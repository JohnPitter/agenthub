...
import { Play, Settings, Users, CheckCircle2 } from "lucide-react";
    ...
    <div className="flex h-full flex-col">
      ...
      <StatBar
        stats={[
          ...
          { label: "Em Progresso", value: tasksByStatus.in_progress, color: "var(--color-warning)" },
          { label: "Em Review", value: tasksByStatus.review, color: "var(--color-purple)" },
          { label: "Concluídas", value: tasksByStatus.done, color: "var(--color-success)" },
          { label: "Concluído", value: `${donePercent}%`, color: "var(--color-brand)" },
        ]}
      />
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-stroke2">
                    {recentTasks.map((task) => {
                      ...
                      return (
                        <tr key={task.id} className="hover:bg-neutral-bg-hover transition-colors">
                          <td className="px-4 py-3">
                            <span className={cn("rounded-md px-2 py-0.5 text-[10px] font-semibold", badge.cls)}>
                              {badge.label}
                            </span>
                          </td>
                          <td className="px-4 py-3 text-[13px] font-medium text-neutral-fg1 truncate max-w-[250px]">
                            {task.title}
                          </td>
                ...
                <div className="divide-y divide-stroke2">
                  {activeAgents.map((agent) => (
                    <div key={agent.id} className="flex items-center gap-3 px-4 py-3">
                      ...
                      <span className="inline-flex items-center gap-1 rounded-full bg-success-light px-2 py-0.5">
